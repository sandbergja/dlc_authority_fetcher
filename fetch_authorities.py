...
vocabularies.add_argument('--lcdgt-only', help='only download demographic group terms authority records', action='store_true')
vocabularies.add_argument('--childrens-only', help='only download childrens subject authority records', action='store_true')
...
try:
    ...
    with open(args.filename) as authorized_terms:
        authorities = {
                'lcnaf': ' scheme:http://id.loc.gov/authorities/names',
                'lcsh': ' scheme:http://id.loc.gov/authorities/subjects',
                'lcgft': ' scheme:http://id.loc.gov/authorities/genreForms',
                'lcdgt': ' scheme:http://id.loc.gov/authorities/demographicTerms',
                'childrens': ' scheme:http://id.loc.gov/authorities/childrensSubjects',
        }
        if args.lcnaf_only:
            suffixes = [authorities['lcnaf']]
        elif args.lcsh_only:
            suffixes = [authorities['lcsh']]
        elif args.lcgft_only:
            suffixes = [authorities['lcgft']]
        elif args.lcdgt_only:
            suffixes = [authorities['lcdgt']]
        elif args.childrens_only:
            suffixes = [authorities['childrens']]
        else:
            suffixes = authorities.values()
        ...
        for term in authorized_terms:
            ...
            for heading_suffix in suffixes:
